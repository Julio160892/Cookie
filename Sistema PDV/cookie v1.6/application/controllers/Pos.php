...
class Pos extends CI_Controller
{
    ...
    public function AddNewSale($type)
    {
        ...
        $waiters = Waiter::find($sale2->waiter_id);
         // nombre de cajero //captura id devuelve nombre
        $posales = Posale::find('all', array(
            'conditions' => array(
                'status = ? AND register_id = ? AND table_id = ?',
                1,
        ...
        <table class="table" cellspacing="0" border="0">
          <thead><tr><th><em>#</em>
            ...
            <th style="text-align:right; width:85px; font-weight:bold;">' . label("SubTotal") . '</th>
          </tr>
          </thead>
        <tbody>';
        ...
        $height = 20;
        $width = 3;
        $ticket .= '</tbody></table><table class="table" cellspacing="0" border="0" style="margin-bottom:8px;">
        <tbody>
        <tr>
        ...
        <td style="text-align:right;font-weight:bold;">' . $sale->subtotal . ' ' . $this->setting->currency . '</td></tr>';
        ...
        switch ($PayMethode[0]) {
            ...
            default:
                ...
                <td colspan="2" style="padding-top:5px; text-align:right; font-weight:bold;">' . number_format((float)(floatval($sale->paid) - floatval($sale->total)), $this->setting->decimals, '.', '') . ' ' . $this->setting->currency . '</td>
                </tr>
                </tbody>
                </table>';
        }
     ...
     */
     public function CloseRegister()
     {
         ...
         <p>' . $date->format('d-m-Y h:i:s') . '</p></blockquote>
         </div>
         ...
         foreach ($waiters as $waiter) {
            ...
            $data .= '<div class="waitercount">
            <ul>
                <li>
                    <h4>'.$waiter->name.' :</h4>
                </li>
                <li>
                <b>' . label("CashinHand") . ' : </b>' . number_format((float)$cachin, $this->setting->decimals, '.', '') . ' ' . $this->setting->currency . '</li>
                <li>
                <b>' . label("Cash") . ' : </b>' . number_format((float)$cashw, $this->setting->decimals, '.', '') . ' ' . $this->setting->currency . '</li> 
                <li>
                <b>' . label("Cheque") . ' : </b>' . number_format((float)$chequew, $this->setting->decimals, '.', '') . ' ' . $this->setting->currency . '</li>
            </ul>
            <div style="clear:both;"></div>
            <div class="wtotal"><h3>' . label("Total") . ' : ' . number_format((float)$Wtotal, $this->setting->decimals, '.', '') . '</h3>
            </div>
            </div>';
         }
    ...
    public function pdfreceipt()
    {
        ...
        $pdf->SetAuthor('Cookie AnthonCode');
...
// solo muestra ticket guardado // no muestra ticket de cada sucursal o tienda
       ...
       $ticket = '<div class="col-md-12">
        ...
        switch ($PayMethode[0]) {
            ...
            default:
                ...
                <td colspan="2" style="padding-top:5px; text-align:right; font-weight:bold;">' . number_format((float)$sale->firstpayement, $this->setting->decimals, '.', '') . ' ' . $this->setting->currency . '</td></tr><tr>
   ...
   public function showticketKit($tableid)
   {
      ...
      <th style="text-align:left; width:180px;">' . label("Product") . '</th>
      <th style="text-align:center; width:50px;">' . label("Quantity") . '</th>
      <th style="text-align:right; width:70px;">' . label("SubTotal") . '</th></tr>
      </thead><tbody>';
      ...
      foreach ($posales as $posale) {
           ...
           <td style="text-align:left; width:180px;">' . $posale->name . '<br>
           <span style="font-size:12px;color:#666">'.$posale->options.'</span></td>
           <td style="text-align:center; width:50px;">' . $posale->qt . '</td>
           <td style="text-align:right; width:70px;font-size:14px; ">' . number_format((float)($posale->qt * $posale->price), $this->setting->decimals, '.', '') . ' ' . $this->setting->currency . '</td></tr>';
           $i ++;
      }
