...
class Reports extends CI_Controller
{
    ...
    public function getCustomerReport()
    {
        ...
        $totals = 0;
        $sales = Sale::find_by_sql("SELECT * FROM `ck_ventas` WHERE client_id = '$client_id' AND created_at between '$start' AND '$end' ORDER BY created_at");
    ...
    public function getRegisterReport()
    {
        ...
        $end = $this->input->post('end');
        $TotalRevenue = 0;
        $register = Register::find_by_sql("SELECT * FROM `ck_registros` WHERE store_id = '$store_id' AND date between '$start' AND '$end' ORDER BY date");
